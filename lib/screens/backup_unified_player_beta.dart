...
/// Player unifié (BETA) : fichier local (audio/vidéo) OU YouTube, avec A/B loop + zoom.
/// - Poignées A/B déplaçables au doigt
/// - Bloc de commandes en bas (play/pause, +/-5s, Replay, Effacer, chevrons A/B)
class UnifiedPlayerBeta extends StatefulWidget {
  final String? initialYoutubeUrl;

  const UnifiedPlayerBeta({super.key, this.initialYoutubeUrl});
...
class _UnifiedPlayerBetaState extends State<UnifiedPlayerBeta> {
  ...
  void _zoomOut() {
    setState(() =>
    _zoomFactor = (_zoomFactor / 1.25).clamp(_minZoom, _maxZoom));
  }
  ...
  void initState() {
    ...
    if (widget.initialYoutubeUrl != null &&
      ...
      final id = YoutubePlayer.convertUrlToId(_ytCtrl.text);
  ...
  Future<void> _restoreLast() async {
    ...
    if (lastUrl != null && lastUrl.isNotEmpty) {
      ...
      final id = YoutubePlayer.convertUrlToId(lastUrl);
      if (id != null) _initYoutube(id);
    }
  ...
  Future<void> _openLocal(String path, {bool autostart = true}) async {
    ...
    if (<String>{'.mp4', '.mov', '.m4v'}.contains(ext)) {
      ...
      _posSub =
          Stream<Duration>.periodic(const Duration(milliseconds: 200), (_) {
  ...
  /// Replay (ex-loupe) : A = B − 4s, boucle ON, seek sur A.
  void _quickLoop() {
    if (_duration == Duration.zero) return;
  ...
  // ---------- TIMELINE avec zoom + poignées drag ----------
  ...
  Widget _timelineCore(double barH) {
    ...
    const double handleTouchW = 32.0; // touch area un peu plus large
    ...
    return LayoutBuilder(
      builder: (ctx, cons) {
        ...
        return Stack(
          ...
          children: [
            // --- SCRUB global (doit être DERRIÈRE les poignées pour ne pas "manger" les drags) ---
            Positioned.fill(
            ...
            // Poignée A — large zone tactile + drag
            if (aX != null)
              Positioned(
            ...
            // Poignée B — large zone tactile + drag
            if (bX != null)
              Positioned(
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          // Replay stylé + Clear juste à côté
          TextButton.icon(
      ...
      body: Column(
        children: [
          // Sélecteur de source + ouvrir local
          Padding(
          ...
          // Timeline avec zoom latéral
          Padding(
            padding: const EdgeInsets.fromLTRB(8, 6, 8, 6),
            child: _timelineThick(),
          ),

          // Pousse tout le bloc de commandes en bas
          const Spacer(),

          // ---- BLOC COMMANDES EN BAS ----
          SafeArea(
            top: false,
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                // Affinage A/B (chevrons)
                Padding(
                    ...
                    children: [
                      ...
                      OutlinedButton.icon(
                        ...
                        style: OutlinedButton.styleFrom(
                          foregroundColor: Colors.white,
                          side: BorderSide(color: _accent.withOpacity(0.9), width: 2),
